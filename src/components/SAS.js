...
const SAS = ({ userId }) => {
  ...
  const buildTrackerMonthData = useCallback((docPayload, trackerName, month, days) => {
    const monthlyData = [];
    for (let day = 1; day <= days; day++) {
      const dayData = { day };
      const monthSlot = docPayload?.[month];
      const rating = monthSlot?.[day - 1]?.rating;
      const value = typeof rating === 'object' ? rating?.rating : rating;
      dayData[trackerName] = value ?? null;
  ...
  useEffect(() => {
    ...
    const cacheKey = `sas-month-${userId}-${year}-${month}`;
    ...
    const unsubscribes = trackerCollections.map((tracker) => {
      ...
      return onSnapshot(
        ...
        (snap) => {
          ...
          const base = buildTrackerMonthData(data, tracker.name, month, days);
          setMonthData((prev) => {
        ...
        (error) => {
          console.error('Error subscribing to SAS tracker', tracker.collection, error);
        }
      );
    });
  ...
  return (
    <div className="sas-monthly-health-tracker">
      <div className="sas-month-navigation month-nav">
      ...
      <div className="sas-legend-grid">
        {[
          { label: 'Sleep', color: '#00a1feff', hint: 'Hours or quality score' },
          { label: 'Anxiety', color: '#ff0000ff', hint: 'Self-rated intensity' },
          { label: 'Stress', color: '#ff9500ff', hint: 'Daily stress check-in' },
        ].map((item) => (
          <div key={item.label} className="sas-legend-card">
            <div className="sas-legend-swatch" style={{ backgroundColor: item.color }} />
            <div className="sas-legend-meta">
              <span className="sas-legend-label">{item.label}</span>
              {/* <span className="sas-legend-hint">{item.hint}</span> */}
            </div>
          </div>
        ))}
      </div>
          ...
          <>
            <ResponsiveContainer width="100%" height={420}>
              <LineChart data={monthData}>
                ...
                />
                {/* <Legend /> */}
                <Line type="monotone" dataKey="Sleep" stroke="#00a1feff" activeDot={{ r: 8 }} />
                <Line type="monotone" dataKey="Anxiety" stroke="#ff0000ff" />
                <Line type="monotone" dataKey="Stress" stroke="#ff9500ff" />
              </LineChart>
            </ResponsiveContainer>
          </>
        ) : (
